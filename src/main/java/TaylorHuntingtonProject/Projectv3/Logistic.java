...
public class Logistic {
	...
	public static void main(String... args) throws FileNotFoundException {
		List<Instance> traindata = readDataSet("trainingdata.csv");
		List<Instance> testdata = readDataSet("testingdataoddhours.csv");
		//3 variables are (1) Spawn Location ID (2) Hour of Spawn and (3) Day of the week
		...
		int correct = 0;
		int listSize = testdata.length();

		for (Instance x in testdata){
			int label = x.label();
			int classifylabel = logistic.classify(x);
			if(label.equals(classifylabel)){
				correct = correct + 1;
			}

		}

		double accuracy = correct / listSize;
		System.out.println("The accuracy for this model was " + (accuracy*100 + "%");
